...
class PartyEmitter(
     ...
     */
    fun createConfetti(deltaTime: Float, party: Party, drawArea: Rect): List<Particle> {
     ...
     */
    private fun createParticle(party: Party, drawArea: Rect): Particle {
        particlesCreated++
        with(party) {
     ...
     */
    private fun getRandomShape(shapes: List<DrawableShape>): DrawableShape {
        val shape = shapes[random.nextInt(shapes.size)]
        val mutatedState =
            shape.drawable.constantState?.newDrawable()?.mutate() ?: shape.drawable
        return shape.copy(drawable = mutatedState)
    }
